...
loff_t aesd_llseek(struct file *filp, loff_t offset, int whence)
{
    ...
    /* Sum the sizes of all entries currently stored in the circular buffer */
    ...
    case SEEK_CUR:
        ...
        break;
    case SEEK_END:
        new_pos = (loff_t)total_size + offset;
        break;
    default:
        mutex_unlock(&dev->lock);
        return -EINVAL;
...
long aesd_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case AESDCHAR_IOCSEEKTO:
        ...
        /* Count how many entries are currently in the buffer */
        if (dev->buffer.full)
            num_entries = AESDCHAR_MAX_WRITE_OPERATIONS_SUPPORTED;
        else
            num_entries = (uint8_t)((dev->buffer.in_offs - dev->buffer.out_offs +
        ...
        actual_idx = (dev->buffer.out_offs + seekto.write_cmd) %
                     AESDCHAR_MAX_WRITE_OPERATIONS_SUPPORTED;

        if (seekto.write_cmd_offset >= dev->buffer.entry[actual_idx].size) {
            mutex_unlock(&dev->lock);
            return -EINVAL;
        }

        /* fpos = sum of sizes of all entries before write_cmd + write_cmd_offset */
        new_pos = 0;
